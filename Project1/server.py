...
class Service(rpyc.Service):
  ...
  def exposed_writelines(self,fil):
    c.writelines(fil)
    
  def exposed_SetCallback3(self,writing):
    c.callback3.writing
...
class ChatServer:
  ...
  def setCallback3(self,writing):
    self.callbacks3 = self.callbacks3 + [writing]
  ...
  def fetch(self,filename):
    ...
    #def open( self, filename, mode = "r" ): 
     #   print "Opening file: " + filename + ", in mode: " + mode 
      #  self.__filehandle = open( filename, mode ) 
       # self.__filename = filename 

  def writelines( self,fil):
    for fn in self.callbacks3:
      try:
        fn(fil)
      except:
        pass
